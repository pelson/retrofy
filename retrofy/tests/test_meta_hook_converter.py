...
from retrofy._meta_hook_converter import (
    MyMetaPathFinder,
    _embedded_runtime_source,
    _EmbeddedRuntimeLoader,
)


def test_embedded_runtime_source_returns_init_for_empty_name():
...
def test_embedded_runtime_source_unknown_returns_none():
    assert _embedded_runtime_source("does_not_exist") is None
...
def test_nested_subpackage_lazy_import_via_meta_hook(tmp_path, monkeypatch):
    ...
    deeper_pkg = sub_pkg / "deeper"
    deeper_pkg.mkdir(parents=True)
    (pkg_root / "__init__.py").write_text("")
    (sub_pkg / "__init__.py").write_text("")
    (deeper_pkg / "__init__.py").write_text("")
    (deeper_pkg / "lazyuser.py").write_text(
        textwrap.dedent(
            """
            lazy import json
    ...
    mod = importlib.import_module("synthpkg.sub.deeper.lazyuser")
    assert mod.loads('{"a": 1}') == {"a": 1}
    assert mod.is_mapping({}) is True
